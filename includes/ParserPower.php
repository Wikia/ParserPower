...
class ParserPower {
	 ...
	 * @return The result of the token replacement within the pattern.
	 */
	static public function applyFieldPattern( $parser, $frame, $inValue, $fieldSep, $tokens, $tokenCount, $pattern ) {
		return self::applyFieldPatternWithIndex( $parser, $frame, $inValue, $fieldSep, '', 0, $tokens, $tokenCount, $pattern );
	}
	 ...
	 * @return The result of the token replacement within the pattern.
	 */
	static public function applyFieldPatternWithIndex( $parser, $frame, $inValue, $fieldSep, $indexToken, $index, $tokens, $tokenCount, $pattern ) {
		$inValue = trim( $inValue );
		if ( trim( $pattern ) !== '' ) { 
			$outValue = $frame->expand( $pattern, PPFrame::NO_ARGS || PPFrame::NO_TEMPLATES );
			if ( $indexToken !== null && $indexToken !== "" ) {
