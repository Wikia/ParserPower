...
class ParserPowerLists {
	...
	private static function explodeList( $sep, $list ) {
		if ( $sep === '' ) {
			$values =
				preg_split( '/(.)/u', $list, -1, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE );
		} else {
	...
	private static function arrayElementTrimUnescape( $inIndex, $inValues ) {
		if ( $inIndex > 0 ) {
			$curOutIndex = 1;
			$count =
				( is_array( $inValues ) || $inValues instanceof Countable ) ? count( $inValues )
					: 0;
		...
		if ( $inList !== '' ) {
			$inSep =
				isset( $params[1] ) ? ParserPower::unescape( trim( $frame->expand( $params[1] ) ) )
					: ',';
			$outSep =
				isset( $params[2] ) ? ParserPower::unescape( trim( $frame->expand( $params[2] ) ) )
					: ', ';
		...
		if ( $inList !== '' ) {
			...
			$outSep =
				isset( $params[2] ) ? ParserPower::unescape( trim( $frame->expand( $params[2] ) ) )
					: ', ';
			$inOffset =
				isset( $params[3] ) ? ParserPower::unescape( trim( $frame->expand( $params[3] ) ) )
					: '';
			$inLength =
			...
			$values =
				self::arrayTrimSliceUnescape(
					$offset,
					$length,
					self::explodeList( $inSep, $inList )
				);
		...
		if ( $list !== '' ) {
			...
			$sep =
				isset( $params[2] ) ? ParserPower::unescape( trim( $frame->expand( $params[2] ) ) )
					: ',';
			$csOption =
				isset( $params[3] ) ? strtolower( trim( $frame->expand( $params[3] ) ) ) : 'ncs';
		...
		if ( $list !== '' ) {
			...
			$sep =
				isset( $params[2] ) ? ParserPower::unescape( trim( $frame->expand( $params[2] ) ) )
					: ',';
			$inOptions =
				isset( $params[3] ) ? strtolower( trim( $frame->expand( $params[3] ) ) ) : '';
				...
				if ( $options & self::INDEX_CS ) {
					for ( $index = $count - 1; $index > -1; --$index ) {
						if ( $values[$index] === $item ) {
							return [
								strval(
									( $options & self::INDEX_NEG ) ? $index - $count : $index + 1
								),
								'noparse' => false,
							];
						}
	...
	public static function lstjoinRender( $parser, $frame, $params ) {
		...
		if ( $inList1 !== '' || $inList2 !== '' ) {
			if ( $inList1 !== '' ) {
				$inSep1 =
					isset( $params[1] ) ? ParserPower::unescape(
						trim( $frame->expand( $params[1] ) )
					) : '';
			...
			if ( $inList2 !== '' ) {
				$inSep2 =
					isset( $params[3] ) ? ParserPower::unescape(
						trim( $frame->expand( $params[3] ) )
					) : '';
	...
	private static function applyPatternWithIndex(
		...
		return ParserPower::applyPatternWithIndex(
			$parser,
			$frame,
			$inValue,
			$indexToken,
	...
	private static function applyFieldPattern(
		...
		return ParserPower::applyFieldPattern(
			$parser,
			$frame,
			$inValue,
			$fieldSep,
	...
	private static function applyTemplate( $parser, $frame, $inValue, $template, $fieldSep ) {
		...
		if ( $inValue != '' ) {
			if ( $fieldSep === '' ) {
				$outValue =
					$frame->virtualBracketedImplode( '{{', '|', '}}', $template, '1=' . $inValue );
			} else {
				$inFields = explode( $fieldSep, $inValue );
				$outFields = [];
				$outFields[] = $template;
				$count =
					( is_array( $inFields ) || $inFields instanceof Countable ) ? count( $inFields )
						: 0;
				for ( $i = 0; $i < $count; $i++ ) {
	 ...
	 * @return array The function output along with relevant parser options.
	 */
	private static function filterListByExclusion(
		$inValues,
		$inSep,
		$values,
		$valueSep,
		...
		if ( $fieldSep !== '' && $tokenSep !== '' ) {
					...
					$result =
						strtolower(
							$parser->replaceVariables(
								ParserPower::unescape( trim( $result ) ),
								$frame
							)
						);
					if ( $result !== 'remove' ) {
			...
			foreach ( $inValues as $value ) {
				if ( trim( $value ) !== '' ) {
					...
					$result =
						strtolower(
							$parser->replaceVariables( ParserPower::unescape( $result ), $frame )
						);
					if ( $result !== 'remove' ) {
						$outValues[] = $value;
					}
	...
	public static function listfilterRender( $parser, $frame, $params ) {
		...
		$default =
			isset( $params["default"] ) ? ParserPower::unescape(
				trim( $frame->expand( $params["default"] ) )
			) : '';

		if ( $inList !== '' ) {
			$keepValues = isset( $params["keep"] ) ? trim( $frame->expand( $params["keep"] ) ) : '';
			$keepSep =
				isset( $params["keepsep"] ) ? trim( $frame->expand( $params["keepsep"] ) ) : ',';
			$keepCS =
				isset( $params["keepcs"] ) ? strtolower(
					trim( $frame->expand( $params["keepcs"] ) )
				) : 'no';
			$removeValues =
				isset( $params["remove"] ) ? trim( $frame->expand( $params["remove"] ) ) : '';
			$removeSep =
				isset( $params["removesep"] ) ? trim( $frame->expand( $params["removesep"] ) )
					: ',';
			$removeCS =
				isset( $params["removecs"] ) ? strtolower(
					trim( $frame->expand( $params["removecs"] ) )
				) : 'no';
			$template =
				isset( $params["template"] ) ? trim( $frame->expand( $params["template"] ) ) : '';
			$inSep =
				isset( $params["insep"] ) ? ParserPower::unescape(
					trim( $frame->expand( $params["insep"] ) )
				) : ',';
			...
			$outSep =
				isset( $params["outsep"] ) ? ParserPower::unescape(
					trim( $frame->expand( $params["outsep"] ) )
				) : ', ';
			...
			$intro =
				isset( $params["intro"] ) ? ParserPower::unescape(
					trim( $frame->expand( $params["intro"] ) )
				) : '';
			$outro =
				isset( $params["outro"] ) ? ParserPower::unescape(
					trim( $frame->expand( $params["outro"] ) )
				) : '';
			...
			if ( count( $outValues ) > 0 ) {
				...
				return [
					self::applyIntroAndOutro( $intro, $outList, $outro, $countToken, $count ),
					'noparse' => false,
				];
			} else {
		...
		if ( $inList !== '' ) {
			...
			$outSep =
				isset( $params[4] ) ? ParserPower::unescape( trim( $frame->expand( $params[4] ) ) )
					: ', ';
			$csOption =
				isset( $params[5] ) ? strtolower( trim( $frame->expand( $params[5] ) ) ) : 'ncs';
	...
	public static function listuniqueRender( $parser, $frame, $params ) {
		...
		if ( $inList !== '' ) {
			$uniqueCS =
				isset( $params["uniquecs"] ) ? strtolower(
					trim( $frame->expand( $params["uniquecs"] ) )
				) : 'no';
			$template =
				isset( $params["template"] ) ? trim( $frame->expand( $params["template"] ) ) : '';
			$inSep =
	...
	private static function generateSortKeysByTemplate(
		...
		foreach ( $values as $value ) {
			$pairedValues[] =
				[ self::applyTemplate( $parser, $frame, $value, $template, $fieldSep ), $value ];
		}
	...
	private static function sortListByKeys(
		...
		if ( $template !== '' ) {
			$pairedValues =
				self::generateSortKeysByTemplate( $parser, $frame, $values, $template, $fieldSep );
		} else {
	...
	public static function listsortRender( $parser, $frame, $params ) {
		...
		if ( $inList !== '' ) {
			...
			$sortOptions =
				isset( $params["sortoptions"] ) ? trim( $frame->expand( $params["sortoptions"] ) )
					: '';
			$subsort =
				isset( $params["subsort"] ) ? strtolower(
					trim( $frame->expand( $params["subsort"] ) )
				) : 'no';
			$subsortOptions =
				isset( $params["subsortoptions"] ) ? trim(
					$frame->expand( $params["subsortoptions"] )
				) : '';
	...
	private static function applyTemplateToList(
		...
		if ( $inList !== '' ) {
			...
			foreach ( $inValues as $inValue ) {
				$outValues[] =
					self::applyTemplate( $parser, $frame, $inValue, $template, $fieldSep );
			}
	...
	public static function listmapRender( $parser, $frame, $params ) {
		...
		if ( $inList !== '' ) {
			...
			$sortMode =
				isset( $params["sortmode"] ) ? strtolower(
					trim( $frame->expand( $params["sortmode"] ) )
				) : 'nosort';
			$sortOptions =
				isset( $params["sortoptions"] ) ? trim( $frame->expand( $params["sortoptions"] ) )
					: '';
		...
		if ( $inList !== '' ) {
			...
			$outSep =
				isset( $params[4] ) ? ParserPower::unescape( trim( $frame->expand( $params[4] ) ) )
					: ', ';
			$sortMode =
				isset( $params[5] ) ? strtolower( trim( $frame->expand( $params[5] ) ) ) : 'nosort';
		...
		if ( $inList !== '' ) {
			...
			$outSep =
				isset( $params[3] ) ? ParserPower::unescape( trim( $frame->expand( $params[3] ) ) )
					: ', ';
			$sortMode =
				isset( $params[4] ) ? strtolower( trim( $frame->expand( $params[4] ) ) ) : 'nosort';
	...
	private static function iterativeListMerge(
		...
		do {
			...
			while ( count( $preValues ) > 0 ) {
				$value1 =
				$matchParams[$valueIndex1] = $mergeParams[$valueIndex1] = array_shift( $preValues );
				$otherValues = $preValues;
				$preValues = [];

				while ( count( $otherValues ) > 0 ) {
					$value2 =
					$matchParams[$valueIndex2] =
					$mergeParams[$valueIndex2] = array_shift( $otherValues );
					$doMerge = call_user_func_array( $applyFunction, $matchParams );
					$doMerge =
						strtolower(
							$parser->replaceVariables(
								ParserPower::unescape( trim( $doMerge ) ),
								$frame
							)
						);
					...
					if ( $doMerge === 'yes' ) {
						...
						$value1 =
							$parser->replaceVariables(
								ParserPower::unescape( trim( $value1 ) ),
								$frame
							);
	...
	private static function mergeListByPattern(
		...
		if ( $inList !== '' ) {
			...
			$matchParams =
				[ $parser, $frame, null, null, $fieldSep, $tokens1, $tokens2, $matchPattern ];
			$mergeParams =
				[ $parser, $frame, null, null, $fieldSep, $tokens1, $tokens2, $mergePattern ];
	...
	public static function listmergeRender( $parser, $frame, $params ) {
		...
		if ( $inList !== '' ) {
			$matchTemplate =
				isset( $params["matchtemplate"] ) ? trim(
					$frame->expand( $params["matchtemplate"] )
				) : '';
			$mergeTemplate =
				isset( $params["mergetemplate"] ) ? trim(
					$frame->expand( $params["mergetemplate"] )
				) : '';
			$inSep =
				isset( $params["insep"] ) ? ParserPower::unescape(
					trim( $frame->expand( $params["insep"] ) )
